...
import { Smartphone, Banknote, Flame, ShoppingBag, ArrowRight } from 'lucide-react';
...
function ServiceCard({ title, sub, icon, status, color }: ServiceCardProps) {
  return (
    <div className={`${color} border-4 border-black p-6 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all cursor-pointer group`}>
      ...
      <h3 className="text-4xl font-black mt-6 leading-none italic uppercase tracking-tighter">{title}</h3>
      <p className="mt-2 font-bold text-xs uppercase opacity-70 tracking-tight">{sub}</p>
